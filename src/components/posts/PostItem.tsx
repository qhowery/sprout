...
const PostItem: React.FC<PostItemProps> = ({
  ...
  const handleDelete = async (
    ...
    try {
      ...
      if (singlePostPage) {
        router.push(`/${post.communityId}`/*r!*/);
      }
    } catch (error: any) {
            ...
            {homePage && (
              <>
                {post.communityImageURL ? (
                  ...
                  <Icon as={LuSprout} fontSize="18px" color="green.500" />
                )}
                <Link href={`${post.communityId}`} /*r!*/>
                  <Text
                    fontWeight={700}
                    _hover={{ textDecoration: "underline" }}
                    onClick={(event) => event.stopPropagation}
                  >{`${post.communityId}`/*r!*/}</Text>
                </Link>
                <Icon as={BsDot} color="gray.500" fontSize={8} />
              </>
            )}
            <Text>
              Posted by {decryptedData.creatorDisplayName/*u!*/}{" "}
